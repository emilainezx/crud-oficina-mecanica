...
async function handleSalvar() {
  try {
    if (idEdicao.value) {
      await atualizarItem(idEdicao.value, formData.value)
    } else {
      await criarItem(formData.value)
    }
      ...
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-slate-200">
          ...
          <tr v-else v-for="item in itens" :key="item.id" class="hover:bg-slate-50 transition-colors">
            ...
            <td class="p-4 text-orange-600 font-bold">R$ {{ Number(item.preco).toFixed(2) }}</td>
