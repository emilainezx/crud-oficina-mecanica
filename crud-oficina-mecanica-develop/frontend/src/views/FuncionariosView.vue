...
async function handleSalvar() {
  try {
    if (idEdicao.value) {
      await atualizarFuncionario(idEdicao.value, formData.value)
    } else {
      await criarFuncionario(formData.value)
    }
      ...
      <table class="w-full text-left border-collapse">
        <thead>
          <tr class="bg-slate-50 border-b border-slate-200 text-slate-600 text-sm uppercase tracking-wider">
            ...
            <th class="p-4 font-semibold">Salário</th>
        ...
        <tbody class="divide-y divide-slate-200">
          ...
          <tr v-else v-for="func in funcionarios" :key="func.id" class="hover:bg-slate-50 transition-colors">
            ...
            <td class="p-4 text-slate-600 text-emerald-600 font-medium">R$ {{ func.salario }}</td>
      ...
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in">
        ...
        <form @submit.prevent="handleSalvar" class="p-5 space-y-4">
          <div>
            ...
            <input type="text" required v-model="formData.nome" class="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-emerald-500 focus:outline-none" placeholder="Ex: Carlos Mecânico" />
          </div>

          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">CPF</label>
              <input type="text" required v-model="formData.cpf" class="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-emerald-500 focus:outline-none" placeholder="000.000.000-00" />
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Telefone</label>
              <input type="text" required v-model="formData.telefone" class="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-emerald-500 focus:outline-none" placeholder="(81) 99999-9999" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" required v-model="formData.cargo" class="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-emerald-500 focus:outline-none" placeholder="Ex: Mecânico Chefe" />
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Salário (R$)</label>
              <input type="number" step="0.01" required v-model="formData.salario" class="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-emerald-500 focus:outline-none" placeholder="3500.00" />
            </div>
          </div>
          ...
          <div class="pt-4 flex justify-end gap-3">
            ...
            <button type="submit" class="px-4 py-2 text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg font-medium shadow-md transition-colors cursor-pointer">
