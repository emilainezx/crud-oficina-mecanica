...
const clientes = ref([]);
const veiculos = ref([]);
const funcionarios = ref([]); // NOVO: Ref para guardar os mecânicos
...
onMounted(async () => {
  try {
    // NOVO: Adicionado api.get("/funcionarios") para puxar os mecânicos
        ...
        <table class="w-full text-left">
          <thead class="bg-slate-50">
            <tr class="text-xs font-bold text-slate-500 uppercase tracking-wider">
              ...
              <th class="p-4">Mecânico</th> <th class="p-4">Status</th>
              <th class="p-4">Valor</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-100">
            <tr v-if="loading" class="text-center p-4 text-slate-500"><td colspan="5" class="p-4">Carregando...</td></tr>
            <tr v-else-if="ordens.length === 0" class="text-center p-4 text-slate-500"><td colspan="5" class="p-4">Nenhuma movimentação registrada.</td></tr>
            <tr v-else v-for="os in ordens.slice(-6).reverse()" :key="os.id" class="hover:bg-slate-50 transition-colors">
