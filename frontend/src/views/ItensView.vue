...
import { PhPackage, PhPencil, PhTrash, PhX, PhMagnifyingGlass } from "@phosphor-icons/vue"
...
async function carregarItens() {
  try {
    const response = await listarItens(termoBusca.value)
  ...
  <div class="animate-fade-in relative">
    ...
    <div class="mb-4 relative">
      ...
      <input
        ...
        class="w-full border border-slate-300 rounded-lg py-2 pl-9 pr-4 focus:ring-2 focus:ring-orange-500 focus:outline-none"
      />
    </div>
      ...
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-slate-200">
          <tr v-if="itens.length === 0">
            <td colspan="6" class="p-8 text-center text-slate-500">Nenhum item encontrado.</td>
          </tr>
    ...
    <div v-if="isModalOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
      <div class="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in">
        <div class="flex justify-between items-center p-5 border-b border-slate-200 bg-slate-50">
          <h3 class="text-xl font-bold text-slate-800">{{ idEdicao ? "Editar Item" : "Cadastrar Novo Item" }}</h3>
        ...
        <form @submit.prevent="handleSalvar" class="p-5 space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" required v-model="formData.nome" class="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="Ex: Pastilha de Freio" />
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Tipo</label>
              <input type="text" v-model="formData.tipo" class="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="Ex: Peça ou Serviço" />
            </div>
          </div>
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Descrição</label>
            <textarea required v-model="formData.descricao" class="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-orange-500 focus:outline-none h-20 resize-none" placeholder="Ex: Pastilha dianteira original." />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Preço (R$)</label>
              <input type="number" step="0.01" min="0" required v-model="formData.preco" class="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="Ex: 150.00" />
            </div>
            <div>
              <label class="block text-sm font-medium text-slate-700 mb-1">Quantidade Inicial</label>
              <input type="number" v-model="formData.quantidade" min="0" class="w-full border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-orange-500 focus:outline-none" placeholder="Ex: 15" />
            </div>
          </div>
          <div class="pt-4 flex justify-end gap-3">
            ...
            <button type="submit" class="px-4 py-2 text-white bg-orange-600 hover:bg-orange-700 rounded-lg font-medium shadow-md transition-colors cursor-pointer">{{ idEdicao ? "Atualizar" : "Salvar" }}</button>
          </div>
        </form>
      </div>
    </div>
